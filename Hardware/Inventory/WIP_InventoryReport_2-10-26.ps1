...
Write-Host "=========================================="
Write-Host "Script started: $($ScriptStart.ToString('yyyy-MM-dd HH:mm:ss'))"
Write-Host "=========================================="
...
#These commands generate files called desktopsAD.csv, usersAD.csv and serverAD.csv at the root of drive C. Updated 6/28/18 - Added additional filter to the server pull to include the wildcard for the registerd symbol (®) in Windows® Small Business Server 2011 Standard - Josh Britton
...
#Check for file, if present remove.

<#
Remove-Item -path "C:\Databranch\desktopsFINAL.csv"
Remove-Item -path "C:\Databranch\desktopsMODELADDED.csv"
Remove-Item -path "C:\Databranch\desktopsMODELS.csv"
Remove-Item -path "C:\Databranch\desktopsSERIAL.csv"
#>
...
# FAST HARDWARE COLLECTION (PowerShell 5.1 compatible)
...
foreach ($name in $Desktops)
{
$JobList += Start-Job -ScriptBlock {
param($n)


    # Skip unreachable machines immediately
    if (!(Test-Connection $n -Count 1 -Quiet -ErrorAction SilentlyContinue)) {
        return
    }

    try {
        $cs  = Get-CimInstance Win32_ComputerSystem -ComputerName $n -Property Manufacturer,Model,SystemType,UserName -ErrorAction Stop
        $bios = Get-CimInstance Win32_BIOS -ComputerName $n -Property SerialNumber -ErrorAction Stop
...
# throttle to 20 concurrent jobs
...
Write-Host "Waiting for hardware queries to finish..." -ForegroundColor Yellow
$results = $JobList | Wait-Job | Receive-Job

# cleanup jobs

$JobList | Remove-Job

# Split to same files expected by original script

$results | Select @{N="ComputerName";E={$_.Name}},UserName,Manufacturer,Model,SystemType |
Export-Csv C:\Databranch\desktopsMODELS.csv -NoTypeInformation -Encoding utf8


$results | Select @{N="ComputerName";E={$_.Name}},SerialNumber |
Export-Csv C:\Databranch\desktopsSERIAL.csv -NoTypeInformation -Encoding utf8
...
Join-Object -Left $CSV1 -LeftJoinProperty Name -Right $CSV2 -RightJoinProperty ComputerName -Type AllInBoth |
Export-Csv C:\Databranch\desktopsMODELADDED.csv -NoTypeInformation -Encoding utf8

$CSV3 = Import-CSV C:\Databranch\desktopsMODELADDED.csv
$CSV4 = Import-CSV C:\Databranch\desktopsSERIAL.csv

Join-Object -Left $CSV3 -LeftJoinProperty Name -Right $CSV4 -RightJoinProperty ComputerName -Type AllInBoth |
Export-Csv C:\Databranch\desktopsFINAL.csv -NoTypeInformation -Encoding utf8
...
# ===========================================
# Script completion timing
# ===========================================
$ScriptEnd = Get-Date
$Duration = $ScriptEnd - $ScriptStart

Write-Host "=========================================="
Write-Host "Script completed: $($ScriptEnd.ToString('yyyy-MM-dd HH:mm:ss'))"
Write-Host "Total runtime: $($Duration.ToString())"
Write-Host "=========================================="
