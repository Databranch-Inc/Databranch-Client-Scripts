<#
This script will copy the ConnectWise Agent locally to a PC, attempt to install, remove failed installs, log steps, and run in a continual loop.
Source of code: https://www.thelazyadministrator.com/2019/04/30/deploy-connectwise-automate-formerly-labtech-agent-remotely-and-quietly-with-powershell/
Customizations/Edits - Josh Britton
5/1/19
1.0 - DB Edits
#>

#Local files to transfer over so it will run locally
#If you change the msi file, change lines 85, 93
$FiletoTransfer = "C:\Databranch\Deployment\Agent_Install.MSI"

#I want to have some basic logging because it will run unattended 
$LogFile = "C:\Databranch\Deployment\Automate Deployment"
...
#Choose one of the following options for gathering machines:
#Parse computers from Active Directory - NOTE: Searchbase = OU for machines
...
Else
    ...
    Else
        ...
        Do
            ...
            Write-Host "Done!" -ForegroundColor White
            $CheckforFile = Invoke-Command -Session $session -ScriptBlock { Test-Path -Path C:\transfer\Agent_Install.msi }
            If ($CheckforFile -eq $True)
                ...
                Do
                    ...
                    Invoke-Command -Session $session -ScriptBlock { Start-Process "msiexec.exe" -ArgumentList "/i C:\Transfer\Agent_Install.msi /q" -Wait }
                    Write-Host "Checking to see if LabTech is installed" -ForegroundColor Yellow
                    $Check = Invoke-Command -Session $session -ScriptBlock { (Get-ChildItem "HKCR:\Installer\Products") | Where-Object { $_.GetValue("ProductName") -like "*LabTech*" } }
                    if ($null -ne $Check)
                        {
