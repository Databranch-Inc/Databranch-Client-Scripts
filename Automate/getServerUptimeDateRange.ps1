...
#* Updated for Databranch use by Josh Britton
#* Email: James.R.Keeler(at)gmail.com
#*
#* Josh Britton - jbritton@databranch.com
#*
#* Params:	[string]$ComputerName - name of the remote computer. If no parameter
#*				is given, the default is localhost
#*
#*			[int]$NumOfDays - numberi of days for which to calculate the 
#*				uptime.  If no parameter is gven, the default is 30 days. The
...
#* Date: 		06/19/2019
#* Time: 		13:54 PM
#* Issue: 		Output for EDF fields is equal to 30 days, can cause misssed dates when running report monthly.
#* Solution:	Changed [int] $NumberOfDays Paramater from (= 30) to (= 31)
#*      	
#*
#*=============================================================================
#* Version:		1.4.1
#* Date: 		08/27/2019
#* Time: 		12:04 PM
#* Issue: 		Can only run this report starting at date called. 
#* Solution:	Changing workflow to allow for begin and end dates:
#*      	
#*
#*=============================================================================
...
.DESCRIPTION
	...
	computer over the past 31 days.
...
.EXAMPLE
	...
	Retrieving shutdown and startup events from MyLaptop for the past 31 days...
	...
	Process {
		...
		if (Test-Connection -ComputerName $ComputerName -Count 1 -TimeToLive 10 -Quiet) {
			...
			if (Test-Path -Path "\\$ComputerName\C$") 
			{
				...
				# If not, we will assume the default, 31 days.  If the value is
				# more than 365, we use 365 as the maximum.
				if ($NumberOfDays -le 0) 
				{
					$NumberOfDays = 31
					$automateFinalOutput.CapturedOutput += "Defaulting to 31 days..."
				} # end if
